...
FormatStreamer_MP3.prototype.__ClientConnected = function (socket) {
    ...
    // do not send old data buffered
    // Flush all data in the buffer
    
    console.log("client connected");
    if (this._SocketListener.clients.size < 2) { 
	console.log("First client connected, flushing data to send: " + this._BurstBuffer.length );
	this._BurstBuffer.length = 0;
    }
...
FormatStreamer_MP3.prototype.__StdInData = function (chunk) {
    // Update burst data
  //while (this._BurstBuffer.length >= this._BurstSize)
  //{    	console.log( "Drop " + this._BurstBuffer.length );
