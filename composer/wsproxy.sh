...
if [ "$USE_CERTBOT_CERTONLY" == "enabled" ]; then
	...
	/usr/local/bin/websockify --key=$PRIVKEY --cert=$CERT --unix-target $X11VNCSOCKET $BIND_INTERFACE
else
	/usr/local/bin/websockify --unix-target $X11VNCSOCKET $BIND_INTERFACE
fi
